...
export default function BlogPageWithCarouselAndFixedTags() {
    ...
    const navItems = [
        { text: 'Về chúng tôi', icon: <HomeIcon />, path: '/about-us', activePaths: ['/'] },
        { text: 'Trang chủ', icon: <BookIcon />, path: '/', activePaths: ['/blog', '/article/:slug', '/tag/:tag'] },
            ...
            <Box sx={{ backgroundColor: currentTheme.palette.background.default, minHeight: '100vh' }}>
                <Paper
                    ...
                    sx={{
                        ...
                        top: 0,
                        zIndex: 1100,
                        backgroundColor: alpha(currentTheme.palette.background.paper, 0.95),
                        backdropFilter: 'blur(8px)',
                        boxShadow: `0 1px 3px ${alpha(currentTheme.palette.text.primary, 0.1)}`
                    }}
                >
                    <Container maxWidth="lg">
                        ...
                        >
                            <MuiLink component={RouterLink} to="/" underline="none" sx={{ display: 'flex', alignItems: 'center', flexShrink: 0 }}>
                                <Box component="img" src="/assets/images/momuni-logo.png" alt="MomUni" sx={{ height: { xs: 28, sm: 32, md: 36 }, mr: 1 }} />
                                <Typography variant="h5" component="div" sx={{ color: currentTheme.palette.primary.main, fontWeight: 'bold', display: { xs: 'none', sm: 'block' }, fontSize: { sm: '1.2rem', md: '1.4rem' } }}>
                                    MomUni
                                </Typography>
                            </MuiLink>
                            ...
                            <Box sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center', gap: { md: 2, lg: 2.5 }, ml: 'auto' }}>
                                {navItems.map((item) => (
                                    <MuiLink
                                        ...
                                        sx={{
                                            ...
                                            '&:hover': {
                                                color: 'primary.dark',
                                                backgroundColor: alpha(currentTheme.palette.primary.light, 0.25)
                                            },
                                            position: 'relative',
                                        }}
                                    >
                                ...
                                <Button
                                    ...
                                    sx={{
                                        ...
                                        boxShadow: `0 3px 8px ${alpha(currentTheme.palette.primary.main, 0.3)}`,
                                        transition: 'background-color 0.2s ease, box-shadow 0.2s ease',
                                        '&:hover': { bgcolor: 'primary.dark', boxShadow: `0 1px 4px ${alpha(currentTheme.palette.primary.dark, 0.4)}` }
                                    }}
                                >
                                    Liên hệ
                                </Button>
                            ...
                            <IconButton
                                sx={{ display: { xs: 'flex', md: 'none' }, ml: 1, color: currentTheme.palette.text.primary }}
                                aria-label="menu"
                        ...
                        <Box
                            sx={{
                                ...
                                gap: { xs: 1.5, sm: 2 },
                                py: 1.5,
                                borderTop: `1px solid ${alpha(currentTheme.palette.divider, 0.7)}`,
                            }}
                        >
                            <TextField
                                fullWidth
                                ...
                                InputProps={{
                                    startAdornment: (
                                        <InputAdornment position="start">
                                            <SearchIcon sx={{ color: currentTheme.palette.text.secondary }} fontSize="small" />
                                        </InputAdornment>
                                    ),
                                    sx: {
                                        borderRadius: '8px',
                                        ...
                                        transition: 'background-color 0.2s ease, box-shadow 0.2s ease',
                                        '&:hover': {
                                            backgroundColor: alpha(currentTheme.palette.background.default, 1),
                                        },
                                        '&.Mui-focused': {
                                            backgroundColor: currentTheme.palette.background.paper,
                                            boxShadow: `0 0 0 2px ${alpha(currentTheme.palette.primary.main, 0.2)}`
                                        },
                                        '.MuiOutlinedInput-notchedOutline': {
                                            borderColor: alpha(currentTheme.palette.grey[400], 0.5),
                                        },
                                        '&:hover .MuiOutlinedInput-notchedOutline': {
                                            borderColor: alpha(currentTheme.palette.grey[500], 0.7),
                                        },
                            ...
                            <TextField
                                ...
                                SelectProps={{
                                    IconComponent: (props) => <FilterListIcon {...props} sx={{ fontSize: '1.2rem', color: currentTheme.palette.text.secondary, mr: 0.5 }} />, // Custom styling for icon
                                    displayEmpty: true,
                                    MenuProps: { PaperProps: { sx: { borderRadius: '8px', mt: 0.5, boxShadow: currentTheme.shadows[3] } } }
                                }}
                                renderValue={(selected) => {
                                    if (selected === "Tất cả") {
                                        return <Box sx={{ display: 'flex', alignItems: 'center', fontSize: '0.9rem', color: currentTheme.palette.text.secondary }}>Lọc theo Tag</Box>;
                                    }
                                    return <Typography variant="body2" sx={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: '0.9rem', color: currentTheme.palette.text.primary }}>{selected}</Typography>;
                                }}
                            >
                                {Array.isArray(allTagsFromData) && allTagsFromData.map((tag) => (
                                    <MenuItem key={tag} value={tag} sx={{ fontSize: '0.9rem', '&:hover': { backgroundColor: alpha(currentTheme.palette.primary.light, 0.15) }, '&.Mui-selected': { backgroundColor: alpha(currentTheme.palette.primary.light, 0.3), fontWeight: 600, '&:hover': { backgroundColor: alpha(currentTheme.palette.primary.light, 0.4) } } }}>
                                        {tag === "Tất cả" ? (
                                            <Box sx={{ display: 'flex', alignItems: 'center', color: currentTheme.palette.text.secondary, fontStyle: 'italic' }}>
                ...
                <Drawer
                    ...
                    PaperProps={{ sx: { width: { xs: '260px', sm: '300px' }, borderTopLeftRadius: '12px', borderBottomLeftRadius: '12px', backgroundColor: currentTheme.palette.background.paper, boxShadow: currentTheme.shadows[5] } }}
                >
                    <Box sx={{ p: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${currentTheme.palette.divider}` }}>
                        <MuiLink component={RouterLink} to="/" underline="none" onClick={handleMobileMenuClose} sx={{ display: 'flex', alignItems: 'center' }}>
                            ...
                            <Typography variant="h6" sx={{ color: currentTheme.palette.primary.main, fontWeight: 'bold' }}>
                                MomUni
                            </Typography>
                        </MuiLink>
                            ...
                            <ListItem key={item.text} disablePadding sx={{ mb: 0.5 }}>
                                <ListItemButton
                                    ...
                                    sx={{
                                        ...
                                        '&:hover': {
                                            backgroundColor: alpha(currentTheme.palette.primary.light, 0.1),
                                        }
                                    }}
                                >
                ...
                <Container maxWidth="lg" sx={{ pt: { xs: 2, md: 3 }, pb: { xs: 2, md: 3 } }}>


                    <Box sx={{ textAlign: 'center', mb: { xs: 2.5, md: 4 } }}>
                        <Typography variant="h1" component="h2"
                            sx={{ fontWeight: 'bold', position: 'relative', display: 'inline-block', pb: 0.5, fontSize: { xs: '2rem', md: '2.6rem' }, color: currentTheme.palette.text.primary }} >
                            ...
                            <Box sx={{ position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '30%', height: '4px', backgroundColor: currentTheme.palette.primary.main, borderRadius: '2px' }} />
                        </Typography>
                    </Box>
                    {featuredCarouselPosts.length > 0 && (!headerSearchQuery && selectedHeaderTag === "Tất cả") && (
                        <MotionBox variants={sectionVariants} initial="hidden" animate="visible" sx={{ mb: { xs: 3, md: 4 } }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                                ...
                                <Typography variant="h2" sx={{ fontSize: { xs: '1.3rem', md: '1.5rem' }, color: currentTheme.palette.text.primary }}>
                                    Bài viết nổi bật
                                </Typography>
                            </Box>
                            <Swiper
                                ...
                                spaceBetween={20}
                                slidesPerView={1}
                                loop={featuredCarouselPosts.length > (featuredCarouselPosts.length >= 3 ? 3 : featuredCarouselPosts.length) && featuredCarouselPosts.length > 1} // loop if more than 3 or less if not enough slides
                                ...
                                style={{ paddingBottom: '40px', '--swiper-pagination-color': currentTheme.palette.primary.main, '--swiper-navigation-color': currentTheme.palette.primary.main, '--swiper-navigation-size': '28px' }}
                            >
                    ...
                    <Grid container spacing={{ xs: 2.5, md: 3.5 }}>
                            ...
                            {currentRegularNewsPosts.length > 0 ? (
                                <Grid container spacing={{ xs: 2.5, sm: 3 }}>
                                    {currentRegularNewsPosts.map(post => (
                                        <Grid item xs={12} sm={6} key={`regular-${post.id}`}>
                                            <BlogCard post={post} small={false} />
                                        </Grid>
                                    ))}
                                </Grid>
                        ...
                        <Grid item xs={12} md={4}>
                            <Paper sx={{ p: { xs: 2, md: 2.5 }, position: 'sticky', top: '145px', borderRadius: '12px', backgroundColor: alpha(currentTheme.palette.background.paper, 0.9), backdropFilter: 'blur(5px)', boxShadow: currentTheme.shadows[2] }}>
                                <Typography variant="h5" gutterBottom sx={{ fontWeight: 'bold', mb: 1.5, color: currentTheme.palette.primary.dark, fontSize: '1.1rem' }}>Đọc nhiều nhất</Typography>
                                {popularPosts.length > 0 ? popularPosts.map((post, idx) => (<Box key={`popular-${post.id}`} sx={{ display: 'flex', mb: 1.5, alignItems: 'flex-start' }}> <Typography variant="h4" sx={{ color: currentTheme.palette.secondary.main, mr: 1.5, lineHeight: 1.2, fontSize: '1.2rem', fontWeight: 600 }}>{idx + 1}.</Typography> <MuiLink component={RouterLink} to={`/article/${post.slug}`} underline="hover" sx={{ fontWeight: 500, fontSize: '0.875rem', lineHeight: 1.4, color: 'text.primary', '&:hover': { color: 'primary.main' } }}> {post.title} </MuiLink> </Box>)) : <Typography sx={{ fontSize: '0.85rem', color: 'text.secondary' }}>Chưa có dữ liệu.</Typography>}
                                <Divider sx={{ my: 2.5 }} />
                                <Typography variant="h5" gutterBottom sx={{ fontWeight: 'bold', mb: 1.5, color: currentTheme.palette.primary.dark, fontSize: '1.1rem' }}>Tags nổi bật</Typography>
                                {Array.isArray(allTagsFromData) && allTagsFromData.filter(t => t !== "Tất cả").length > 0 ? (<Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.8 }}> {allTagsFromData.filter(t => t !== "Tất cả").slice(0, 10).map(tag => (<Chip key={tag} label={tag} component="a" href={`/tag/${tag.toLowerCase().replace(/\s+/g, '-')}`} clickable size="small" sx={{ backgroundColor: alpha(currentTheme.palette.secondary.light, 0.3), color: currentTheme.palette.secondary.dark, fontWeight: 500, '&:hover': { backgroundColor: currentTheme.palette.secondary.main, color: 'white' } }} />))} </Box>) : <Typography sx={{ fontSize: '0.85rem', color: 'text.secondary' }}>Chưa có tags.</Typography>}
                            </Paper>
                        </Grid>
                    </Grid>
                </Container>
                ...
                {exploreMorePosts.length > 0 && (
                    <Container maxWidth="lg" sx={{ py: { xs: 2.5, md: 4 }, borderTop: `1px solid ${alpha(currentTheme.palette.primary.main, 0.15)}`, mt: 3.5 }}>
                        <MotionBox variants={sectionVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2.5 }}>
                                ...
                                <Typography variant="h2" sx={{ fontSize: { xs: '1.3rem', md: '1.5rem' }, color: currentTheme.palette.text.primary }}>Có thể bạn quan tâm</Typography>
                            </Box>
                            <Swiper modules={[Navigation, SwiperAutoplay, SwiperPagination]} spaceBetween={20} slidesPerView={2} loop={exploreMorePosts.length > (exploreMorePosts.length >= 4 ? 4 : exploreMorePosts.length) && exploreMorePosts.length > 1} autoplay={{ delay: 6000, disableOnInteraction: false }} navigation={exploreMorePosts.length > 1} pagination={{ clickable: true, dynamicBullets: true }} breakpoints={{ 600: { slidesPerView: Math.min(exploreMorePosts.length, 3), spaceBetween: 20 }, 900: { slidesPerView: Math.min(exploreMorePosts.length, 4), spaceBetween: 20 }, 1200: { slidesPerView: Math.min(exploreMorePosts.length, 4), spaceBetween: 24 }, }} style={{ paddingLeft: '5px', paddingRight: '5px', paddingBottom: '30px', '--swiper-pagination-color': currentTheme.palette.primary.main, '--swiper-navigation-color': currentTheme.palette.primary.main, '--swiper-navigation-size': '28px' }} >
                ...
                <Box sx={{ position: 'fixed', bottom: '20px', right: '20px', display: 'flex', flexDirection: 'column', gap: 1.5, zIndex: 1200 }}>
                    <Fab color="primary" size="medium" aria-label="call" sx={{ boxShadow: currentTheme.shadows[4] }}> <CallIcon /> </Fab>
                    <Fab sx={{ backgroundColor: currentTheme.palette.secondary.main, color: 'white', '&:hover': { backgroundColor: currentTheme.palette.secondary.dark }, boxShadow: currentTheme.shadows[4] }} size="medium" aria-label="chat"> <ChatIcon /> </Fab>
                    <Fab sx={{ backgroundColor: alpha(currentTheme.palette.text.secondary, 0.7), color: 'white', '&:hover': { backgroundColor: currentTheme.palette.text.primary }, boxShadow: currentTheme.shadows[3] }} size="small" aria-label="scroll-to-top" onClick={scrollToTop}> <ArrowUpwardIcon fontSize="small" /> </Fab>
                </Box>
            </Box>
        </ThemeProvider>
    );
